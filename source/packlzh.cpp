...
INTERN inline bool check_plz_header( void );
INTERN inline bool read_plz_header( void );
INTERN inline bool write_plz_header( void );
...
INTERN int lim_count		= ( 1 <<  8 ); // limit for LZ count
INTERN int lim_dist			= ( 1 << 12 ); // limit for LZ distance
INTERN int lzh_blocksize 	= ( 1 << 16 ); // size of one LZH block
...
#endif
INTERN const char   cmp_magic[] = { 'L', 'S' };
...
#if defined(BUILD_LIB)
EXPORT bool plzlib_convert_stream2stream( char* msg )
{
	// process in main function
	return plzlib_convert_stream2mem( NULL, NULL, msg ); 
}
#endif
...
#if defined(BUILD_LIB)
EXPORT bool plzlib_convert_file2file( char* in, char* out, char* msg )
{
	// init streams
	plzlib_init_streams( (void*) in, 0, 0, (void*) out, 0 );
...
#if defined(BUILD_LIB)
EXPORT bool plzlib_convert_stream2mem( unsigned char** out_file, unsigned int* out_size, char* msg )
{
	clock_t begin, end;
	int total;
	float cr;	
...
#if defined(BUILD_LIB)
EXPORT void plzlib_init_streams( void* in_src, int in_type, int in_size, void* out_dest, int out_type )
{
	...
	if ( !force_enc ) {
		...
		if ( (buffer[0] == cmp_magic[0]) && (buffer[1] == cmp_magic[1]) ) {
			// file might be PLZ - check version and header
			if ( check_plz_header() ) {
				filetype = F_PLZ;
				...
				strcpy( cmpfilename, (  in_type == 0 ) ? (char*) in_src   : "PLZ in memory" );
	...
	if ( filetype == F_UNK ) {
		...
		strcpy( uncfilename, (  in_type == 0 ) ? (char*) in_src   : "PLZ in memory" );
...
#if defined(BUILD_LIB)
EXPORT void plzlib_force_encoding( bool setting )
{
	// adopt the setting
	force_enc = setting;
}
...
#if defined(BUILD_LIB)
EXPORT const char* plzlib_version_info( void )
{
...
#if defined(BUILD_LIB)
EXPORT const char* plzlib_short_name( void )
{
...
{	
	...
	while ( --argc > 0 ) {
		...
		else if ( sscanf( (*argv), "-lc%i", &i ) == 1 ) {			
			i = ( i < 1 ) ? 1 : i;
			i = ( i > 8 ) ? 8 : i;
			lim_count = 1 << i;
		}
		else if ( sscanf( (*argv), "-ld%i", &i ) == 1 ) {			
			i = ( i < 0 ) ? 0 : i;
			i = ( i > LZ_D_LMAX ) ? LZ_D_LMAX : i;
			lim_dist = 1 << i;
		}
		else if ( sscanf( (*argv), "-lb%i", &i ) == 1 ) {			
			i = ( i < 0 ) ? 0 : i;
			lzh_blocksize = 1 << i;
		}
		#endif
...
{	
	...
	if ( developer ) {
	fprintf( msgout, "\n" );
	fprintf( msgout, " [-lc?]   log limit for LZ count (max: 8) (def: 8)\n" );
	fprintf( msgout, " [-ld?]   log limit for LZ distance (max: %i) (def: 12)\n", LZ_D_LMAX );
	fprintf( msgout, " [-lb?]   log size of coded block (def: 16)\n" );
...
{	
	...
	// check if this is a compressed plz file
	while ( !force_enc ) {
		// read file id and version number
		if ( str_in->read( fileid, 1, 3 ) != 3 ) break;
		// check magic number
		if ( ( fileid[0] != cmp_magic[0] ) || ( fileid[1] != cmp_magic[1] ) ) break;
		// check plz header, extract file name
		if ( !check_plz_header() ) break;
		// alright: this is a .plz file!
...
INTERN bool encode_file( void )
{
	...
	if ( !write_plz_header() ) return false;
	
	
	// --- main coding loop ---
	lzh_enc = new lzh_encoder( str_in, lim_count, lim_dist );
	for ( lzhs = 1; lzhs > 0; ) lzhs = lzh_enc->encode_lzh( str_out, lzh_blocksize );
	if ( lzhs < 0 ) {
		sprintf( errormessage, MEM_ERRMSG );
		errorlevel = 2;
		return false;
...
INTERN bool decode_file( void )
{
	...
	// --- main decoding loop ---
	lzh_dec = new lzh_decoder( str_out );
	for ( lzhs = 1; lzhs > 0; ) {
		if ( str_in->getpos() >= cmpfilesize ) break;
		lzhs = lzh_dec->decode_lzh( str_in );
	}
	if ( lzhs < 0 ) {
		sprintf( errormessage, MEM_ERRMSG );
		errorlevel = 2;
...
/* -----------------------------------------------
	check plz header, extract uncompressed name
	----------------------------------------------- */
INTERN inline bool check_plz_header( void )
{
...
INTERN inline bool read_plz_header( void )
{
	...
	// read number of total bytes, do hashing
...
INTERN inline bool write_plz_header( void )
{
	...
	// write number of total bytes, do hashing
